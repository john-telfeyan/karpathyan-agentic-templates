...
this `wiki/` folder (processed knowledge) → this index, with `../log.md` recording each ingest.

## The three layers

- **`../raw/`** — immutable source documents. Never edit; treat as frozen inputs.
- **`./` (this `wiki/` folder)** — processed knowledge. One markdown page per concept,
  cross-linked with `[[page-name]]` (the filename without `.md`).
...
2. Open the relevant sibling `*.md` page in this folder and follow its `[[links]]`.
